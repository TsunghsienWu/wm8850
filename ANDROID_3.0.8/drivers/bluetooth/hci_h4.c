...
static int h4_enqueue(struct hci_uart *hu, struct sk_buff *skb)
{
	...
	if(is_mtk6622()){
		...
		if((jiffies_to_msecs(jiffies) - lCurrentTime) > 5 * 1000){
			BT_INFO("h4_enqueue idle more than 5s 0x%08lx 0x%08lx", 
			...
			if (!skbMagic) {
				BT_ERR("Can't alloc memory for magic packet"); /* how to handle? */
				return 0;
			}
			skbMagic->pkt_type = 1;
...
static int h4_recv(struct hci_uart *hu, void *data, int count)
{
	if(!is_mtk6622()){
		...
		while (count) {
			...
			if(while_count > 5000){
				BT_ERR("while_count %d", while_count);
			}
					...
					if(HCI_EVENT_PKT == bt_cb(h4->rx_skb)->pkt_type){
						...
						case HCI_EV_CMD_COMPLETE:
							...
							if(usOpCode == 0xFCC0){
								...
								/* flag is cleared, resume TX. or later? */
								hci_uart_tx_wakeup(hu);
							}

							if(usOpCode == 0xFCC1){
								BT_DBG("Recv host wakeup event 0x%04x\n", (int)usOpCode);
								//mt_bt_enable_irq();
							}
							break;
						}
			...
			if (!h4->rx_skb) {
				BT_ERR("Can't alloc memory for new packet");
				h4->rx_state = H4_W4_PACKET_TYPE;
				h4->rx_count = 0;
				return 0;
			}
