...
//	printk(KERN_INFO "gri wmt_ep_enable() %s\n", ep ? ep->ep.name : NULL);
	...
	if (!_ep || !desc || ep->desc
		...
		printk("ep->bEndpointAddress = 0x%08X\n", ep->bEndpointAddress);
		printk("desc->bEndpointAddres = 0x%08X\n", desc->bEndpointAddress);
		printk("ep->maxpacket =0x%08X\n", ep->maxpacket);
		printk("desc->wMaxPacketSize =0x%08X\n", desc->wMaxPacketSize);

		printk("_ep =0x%08X\n", (unsigned int)_ep);
		printk("desc =0x%08X\n", (unsigned int)desc);
		printk("ep->desc =0x%08X\n", (unsigned int)ep->desc);
	...
	if ((desc->bmAttributes == USB_ENDPOINT_XFER_BULK
			...
			|| !desc->wMaxPacketSize) {
		printk("[wmt_ep_enable]bad %s maxpacket\n", _ep->name);
		return -ERANGE;
	}
...
//	unsigned long	flags;
	...
	if (!_ep || !ep->desc) {
		printk("[wmt_ep_disable], %s not enabled\n",
			_ep ? ep->ep.name : NULL);
		return -EINVAL;
	}
...
static void pullup_enable(struct vt8500_udc *udc)
{
	...
	/** connect to adapter, charge with large electricity */
	if (wmt_batt_charge_type() == 1 && udc_wmt_suspend == 0) {
		printk(KERN_INFO "pullup_enable: connect to adapter, charge with large electricity\n");
...
//		return -EINVAL;        
	...
	if (!_req || !req->req.complete || !req->req.buf
		...
		printk("[wmt_ep_queue], bad params\n");
		return -EINVAL;
	}
	if (!_ep || (!ep->desc && ep->bEndpointAddress)) {
		printk("[wmt_ep_queue], bad ep\n");
		return -EINVAL;
	}
...
// unsigned long flags;
 ...
 spin_lock_irqsave(&ep->udc->lock, irq_flags);
  ...
  DBG("wmt_ep_dequeue() %s,%d\n", ep ? ep->ep.name : NULL,ep->bEndpointAddress);
 ...
 if (((ep->bEndpointAddress & 0x7F) != 0) && ep->queue.next == &req->queue) {
  ...
  if (ep->bEndpointAddress == 1){
   ...
   pDevReg->Bulk1DesStatus &= 0xF0;
   {
    ...
    /* set endpoint data toggle*/
    btmp = pDevReg->Bulk1DesTbytes2 & 0x80;
   ...
   pDevReg->Bulk3DesStatus &= 0xF0;
   {
    ...
    /* set endpoint data toggle*/
    btmp = pDevReg->Bulk3DesTbytes2 & 0x80;
...
static void pullup_disable(struct vt8500_udc *udc)
{
	...
	/** disconnect, set default value */
	if (wmt_batt_charge_type() == 1 && udc_wmt_suspend == 0) {
		pr_info("pullup_disable:no connect, charge with small electricity\n");
