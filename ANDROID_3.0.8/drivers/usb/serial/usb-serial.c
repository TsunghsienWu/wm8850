			       ...
			       const struct usb_device_id *id)
{
	...
	if(((le16_to_cpu(dev->descriptor.idVendor) == 0x231e) &&
		(le16_to_cpu(dev->descriptor.idProduct) == 0x0036)) //CYIT
		||((le16_to_cpu(dev->descriptor.idVendor) == 0x15eb) &&
		(le16_to_cpu(dev->descriptor.idProduct) == 0x0001)&&is_via_4wire())//VIA Telecom
		||((le16_to_cpu(dev->descriptor.idVendor) == 0x12d1) &&
		(le16_to_cpu(dev->descriptor.idProduct) == 0x1001)&&is_modem_onboard())//huawei mu509
		||((le16_to_cpu(dev->descriptor.idVendor) == 0x201e) &&
		(le16_to_cpu(dev->descriptor.idProduct) == 0x1022)&&is_via_4wire())//(haier)VIA Telecom
		)	
	{
		startIndex = 10;
	}
