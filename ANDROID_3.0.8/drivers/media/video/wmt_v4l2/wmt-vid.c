...
static void vid_gpio_init(vid_mode mode)
{
    ...
    GPIO_OC_GP31_BYTE_VAL &= ~BIT0;
    GPIO_PIN_SHARING_SEL_4BYTE_VAL |= BIT11;// 1,  24MHZ output  0 , SPISS 0
    REG32_VAL(GPIO_CTRL_GP28_NAND_CFG_0_BYTE_ADDR) &= ~BIT28;// 1,  24MHZ output  0 , SPISS 0
...
static int i2c_xfer_dbg = 1;

void wmt_vid_i2c_xfer_dbg(int enable)
{
	i2c_xfer_dbg = enable;
}
...
* \retval  0 if success
*/ 
int wmt_vid_i2c_write16addr16data(int chipId ,unsigned int index,unsigned int data)
{
	int ret=0;
	struct i2c_msg msg[1];
	unsigned char buf[4];
    if (vid_i2c_gpio_en)
    {
    	DBG_ERR("wmt_vid_i2c_write16addr16data() Not support GPIO mode now");
    	return -1;
    }
	buf[0]=((index>>8) & 0xff);
	buf[1]=(index & 0xff);
...
int wmt_vid_i2c_read16addr16data(int chipId ,unsigned int index)
{
	int ret=0;
...
int wmt_vid_i2c_init( short dev_id)
{
	...
	int indexAdapter=2;//by flashchen use i2c2 as default
	int varlen;
   	char varbuf[128];
	memset(varbuf, 0, sizeof(varbuf));
	varlen = sizeof(varbuf);
    	if (!wmt_getsyspara("wmt.cmos.i2cindex", varbuf, &varlen)) {
		if(varbuf[0]=='1')
			indexAdapter=1;
		else if(varbuf[0]=='2')
			indexAdapter=2;
	...
	adapter = i2c_get_adapter(indexAdapter);
	if (adapter == NULL) {
...
int wmt_vid_i2c_release(void)
{
	...
	//i2c_del_driver(&wmt_vid_i2c_driver);
...
int wmt_vid_open(vid_mode mode, cmos_uboot_env_t *uboot_env)
{
    ...
    if(mode == VID_MODE_CMOS) {
        ...
        VID_REG_SET32( REG_VID_INT_CTRL, int_ctrl );
          VID_REG_SET32( REG_VIDCLK_INV, 0 );
...
EXPORT_SYMBOL(wmt_vid_i2c_write16addr16data);
EXPORT_SYMBOL(wmt_vid_i2c_read16addr16data);
...
EXPORT_SYMBOL(wmt_vid_i2c_xfer_dbg);
