...
int get_cmos_uboot_env(void)
{
	...
	varlen = sizeof(varbuf2);
		...
		n = sscanf(varbuf2, "%d:%d:%d:%d:%d:%d:%d:%d:%d:%d",
					&env_param[0].type, &env_param[0].control, &env_param[0].level, &env_param[0].rotate, &env_param[0].others,
					&env_param[1].type, &env_param[1].control, &env_param[1].level, &env_param[1].rotate, &env_param[1].others);
		...
		{
			env_param[0].facing=0;
			env_param[1].facing=1;
		}
	}
	env_param[0].cmosid=CMOS_ID_NONE;
	env_param[1].cmosid=CMOS_ID_NONE;
#if 0// switch cameraid in app		 
	if(!env_param[0].type && env_param[1].type)
	{
		memcpy(&env_param[0],&env_param[1],sizeof(struct envParam));
		memset(&env_param[1],0,sizeof(struct envParam));
	}
#endif
...
int cmos_cam_g_fmt_cap(struct file *file, void *fh, struct v4l2_format *f)
{
	...
	dev->cmosid=cmos_init_ext_device(&init_arg,CMOS_ID_NONE);//fake init just get cmos id

	if(CMOS_ID_OV_5640==dev->cmosid )
	{
		//drv->width  = 2592;
		//drv->height = 1944;
		drv->width  = 2592;
	...
	else if(CMOS_ID_OV_3660==dev->cmosid || CMOS_ID_OV_3640==dev->cmosid || CMOS_ID_SAMSUNG_S5K5CA==dev->cmosid )
	{
		drv->width  = 2048;
		drv->height = 1536;
	}
	else if(CMOS_ID_YACD_511==dev->cmosid || CMOS_ID_GC_2015==dev->cmosid 
			|| CMOS_ID_GC_2035==dev->cmosid 
			|| CMOS_ID_NT_99250==dev->cmosid 
			|| CMOS_ID_SID_130B==dev->cmosid 
			|| CMOS_ID_OV_2643==dev->cmosid
			|| CMOS_ID_OV_2659==dev->cmosid )
	{
		drv->width  = 1600;
		drv->height = 1200;
	}
	...
	f->fmt.pix.priv=dev->cmosid;
    //DBG_ERR("Not support now!\n");
	if(CMOS_ID_NONE==dev->cmosid)
		return -1;
	else
		return 0;
}
...
int cmos_cam_s_fmt_cap(struct file *file, void *fh, struct v4l2_format *f)
{
    ...
    TRACE("Enter\n");
	fastswitch = f->fmt.pix.priv & 0xf0; // enable which device
	cameraid = f->fmt.pix.priv & 0xf; // enable which device
	drv->width  = f->fmt.pix.width;
	drv->height = f->fmt.pix.height;
	drv->frame_size = ALIGN64(drv->width) * drv->height *2 ;  // *4 for  ARGB display frambuffer
	color_format = f->fmt.pix.pixelformat;

	if ( color_format == V4L2_PIX_FMT_YUYV)
		printk("got pixel format %d \n", color_format);
    ...
    init_arg.width = drv->width;
    init_arg.height = drv->height;
	if(1==env_param[cameraid].type)
	{
		init_arg.cmos_v_flip = env_param[cameraid].rotate & 0x1;
		init_arg.cmos_h_flip = !!(env_param[cameraid].rotate & 0x2);
	}
    ...
    //ret = cmos_init_ext_device(&init_arg,CMOS_ID_NONE);
	if(fastswitch )// this is the sign to know switch mode for taking pic
	{
		init_arg.mode_switch=1;
		cmos_init_ext_device (&init_arg,dev_s.cmosid);
	}
	else
		dev->cmosid=cmos_init_ext_device(&init_arg,CMOS_ID_NONE);
#if 1
	if(CMOS_ID_OV_5640==dev->cmosid )
	{
    	drv->frame_size = ALIGN64(2592) * 1936*	2 ;  // *4 for  ARGB display frambuffer
    	//drv->frame_size = ALIGN64(2592) * 1944*	2 ;  // *4 for  ARGB display frambuffer
	}
	else if(CMOS_ID_OV_3660==dev->cmosid || CMOS_ID_SAMSUNG_S5K5CA==dev->cmosid )
	{
    	drv->frame_size = ALIGN64(2048) * 1536*	2 ;  // *4 for  ARGB display frambuffer
	}
	else if(CMOS_ID_YACD_511==dev->cmosid || CMOS_ID_GC_2015==dev->cmosid 
			|| CMOS_ID_GC_2035==dev->cmosid 
			|| CMOS_ID_NT_99250==dev->cmosid 
			...
			|| CMOS_ID_OV_2659==dev->cmosid )
	{
    	drv->frame_size = ALIGN64(1600) * 1200 * 2 ;  // *4 for  ARGB display frambuffer
	}
	else
	{
    	drv->frame_size = ALIGN64(640) * 480 * 2 ;  // *4 for  ARGB display frambuffer
	}
#endif
...
static int errCount=0;
    ...
    if( drv->streamoff ) {
        ...
        list_for_each(next, &drv->head) {
        //list_for_each_save(next, &drv->head) {
            fb = (cmos_fb_t *) list_entry(next, cmos_fb_t, list);
            if( fb ) {
                pop_queue(drv, fb);    
                break;
    ...
    if( (wait_event_interruptible_timeout( cmos_wait,
		...
		if(errCount > 2)
		{
			cmos_init_ext_device (&init_arg,CMOS_ID_NONE);
			errCount=0;
		}
		return -1;
    }
...
int cmos_cam_s_ctrl(struct file *file, void *fh, struct v4l2_control *a)
{
	...
	{
		case 0:
			updateSceneMode(dev->cmosid,a->value);
			break;
		case 1:
			updateWhiteBalance(dev->cmosid,a->value);
		case 2:
			updateAntiBanding(dev->cmosid,a->value);
			break;
	}
    //DBG_ERR("Not support now!\n");
    return 0;
...
static int cmos_open(struct file *filp)
{
	...
	if(!env_param[0].type && !env_param[1].type)
	{
		printk("please set the camera uboot env\n");
		return -1;
	}
	if(cmos_dev_ref)
		return -EBUSY;
...
static int cmos_release(struct file *filp)
{
	...
	//msleep(200);
    cmos_exit_ext_device(dev->cmosid);
