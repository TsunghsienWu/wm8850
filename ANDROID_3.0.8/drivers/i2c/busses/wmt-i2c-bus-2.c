...
#define USE_UBOOT_PARA    //#undef USE_UBOOT_PARA
...
static int i2c_wmt_resource_init(void)
{
	if (is_master == 0)
		return 0;
	if (request_irq(i2c.irq_no , &i2c_wmt_handler, IRQF_DISABLED, "i2c2", 0) < 0) {
		DPRINTK(KERN_INFO "I2C: Failed to register I2C2 irq %i\n", i2c.irq_no);
		return -ENODEV;
	}
	return 0;
}
