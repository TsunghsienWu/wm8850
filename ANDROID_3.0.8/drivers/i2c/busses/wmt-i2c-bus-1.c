...
)
{
	...
	if(i2c.regs->isr_reg & I2C_ISR_NACK_ADDR)
	{
		mdelay(1);
		if(3== i2c.regs->isr_reg )
		{
			printk("fixed weired interrupt once\n" );//by flashchen
	...
	if (isr_status & I2C_ISR_NACK_ADDR) {
		...
		list_del_init(&fifo_head->busfifohead);/*del request*/
...
static int i2c_wmt_resource_init(void)
{
	if (is_master == 0)
		return 0;
	if (request_irq(i2c.irq_no , &i2c_wmt_handler, IRQF_DISABLED, "i2c1", 0) < 0) {
		DPRINTK(KERN_INFO "I2C: Failed to register I2C1 irq %i\n", i2c.irq_no);
		return -ENODEV;
	}
	return 0;
}
