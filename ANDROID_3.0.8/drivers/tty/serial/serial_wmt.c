...
error_return:
	...
	if(sport->port.line == MUX_INDEX&&cmux_opened()){
					mux_ringbuffer_write(&rbuf, &ch, 1);
...
//	printk("%s %d 0x%x\n",__FUNCTION__,__LINE__,uart);


#ifdef CONFIG_TS0710_MUX_UART
	if( sport->port.line == MUX_INDEX){
...
#ifdef CONFIG_TS0710_MUX_UART
	serial_for_mux_driver = wmt_reg.tty_driver;
...
static int __init wmt_serial_init(void)
{
	...
	char buf[256];
	int varlen = 256;	
	if( wmt_getsyspara("wmt.uart.modem2g",buf,&varlen) == 0) 
	{

		sscanf(buf,"%d",&MUX_INDEX);
	}
