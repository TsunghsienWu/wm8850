...
//kevin modify for pwm3
unsigned int pwm_level[4] = { 0, 0,0,0 };
unsigned int pwm_freq[4] = {16000, 16000 ,1600,1600}; //modified by howayhuo. org: {1000, 1000 }
unsigned int pwm_enable[4] = {0,0,0,0};
...
void set_lcd_power(int on)
{
	...
	//--> added by howayhuo, don't power off LCD
	/*
	if((REG32_VAL(g_lcd_pw_pin.ctl) & val) && (REG32_VAL(g_lcd_pw_pin.oc) & val))
	{
		if(g_lcd_pw_pin.act)
		{
			if(REG32_VAL(g_lcd_pw_pin.od) & val)
				return;
		}
		else
		{
