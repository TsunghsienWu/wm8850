...
#ifdef CONFIG_PM
static int wm8994_suspend(struct i2c_client *i2c, pm_message_t mesg)
{
	...
	dev_dbg(wm8994->dev, "wm8994_suspend enter\n");
...
static int wm8994_resume(struct i2c_client *i2c)
{
	...
	dev_dbg(wm8994->dev, "wm8994_resume enter\n");
				   ...
				   int bytes, const void *src)
{
	...
	if (ARRAY_SIZE(buf) < bytes + 2)
		return -EINVAL;

	//reg = cpu_to_be16(reg);
	//strncpy(buf, &reg, 2);
	//strncpy(&buf[2], src, bytes);
	buf[0] = (reg >> 8) & 0xff;
	buf[1] = reg & 0xff;
	for (i = 0; i < bytes; i++)
		buf[i+2] = *(unsigned char *)((unsigned char *)src + i);
