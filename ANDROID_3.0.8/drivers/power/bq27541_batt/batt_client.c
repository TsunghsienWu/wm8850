...
struct i2c_board_info batt_i2c_board_info = {
	.type          = BATT_I2C_NAME,
	.flags         = 0x00,
	.addr          = BATT_I2C_ADDR,
	.platform_data = NULL,
...
static struct i2c_client *batt_client=NULL;
int batt_i2c_register_device (void)
{
	struct i2c_board_info *sensor_i2c_bi;
	struct i2c_adapter *adapter = NULL;
	//struct i2c_client *client   = NULL;
	sensor_i2c_bi = &batt_i2c_board_info;
	...
	batt_client = i2c_new_device(adapter, sensor_i2c_bi);
	if (batt_client == NULL) {
...
void batt_i2c_unregister_device(void)
{
	if (batt_client != NULL)
	{
		i2c_unregister_device(batt_client);
	}
}
struct i2c_client* batt_get_i2c_client(void)
{
	return batt_client;
}
//EXPORT_SYMBOL_GPL(batt_i2c_register_device);