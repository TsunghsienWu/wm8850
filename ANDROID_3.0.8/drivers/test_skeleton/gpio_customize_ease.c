...
//#define REG8_VAL(x) (0x5a5a5a5a&x)
...
*/
int gpio_enable(GPIO_CTRL *gp_ctrl, GPIO_IN_OUT mode)
{
...
* disable gpio mode
*/
int gpio_disable(GPIO_CTRL *gp_ctrl)
{
	REG8_VAL(gp_ctrl->gpio_enable.address) &= ~(1<<gp_ctrl->bitmap);
	return 0;
}
int gpio_set_value(GPIO_CTRL *gp_ctrl,GPIO_OUTPUT value)
{
...
int gpio_get_value(GPIO_CTRL *gp_ctrl)
{
...
*/
int gpio_pull_enable(GPIO_CTRL *gp_ctrl,GPIO_PULL pull_mode)
{
	...
	printf("in %s offset:%d\n",__func__,offset);
	if(pull_mode == PULL_UP)
	{
		REG8_VAL(gp_ctrl->gpio_pull_ctrl.address) |= (1<<offset);
		printf("gp_ctrl->gpio_pull_ctrl:%x\n",REG8_VAL(gp_ctrl->gpio_pull_ctrl.address));
	} else {
		REG8_VAL(gp_ctrl->gpio_pull_ctrl.address) &= ~(1<<offset);
		printf("pull donw branch\n");
	}
	return 0;
}
int gpio_pull_disable(GPIO_CTRL *gp_ctrl)
{
	REG8_VAL(gp_ctrl->gpio_pull_enable.address) &= ~(1<<gp_ctrl->bitmap);
	return 0;
}
...
*/
int enable_gpio_int(GPIO_CTRL *gp_ctrl, GPIO_INT_ENA enable)
{
...
int set_gpio_irq_triger_mode(GPIO_CTRL *gp_ctrl, GPIO_IRQ_TRIGER_MODE trigger_mode)
{
...
int gpio_irq_state(GPIO_CTRL *gp_ctrl)
{
...
*/
int gpio_clean_irq_status(GPIO_CTRL *gp_ctrl)
{
	REG8_VAL((gp_ctrl->gpio_int_status.address)) |= (1<<gp_ctrl->bitmap);
}

int gpio_irq_isEnable(GPIO_CTRL *gp_ctrl)
{
