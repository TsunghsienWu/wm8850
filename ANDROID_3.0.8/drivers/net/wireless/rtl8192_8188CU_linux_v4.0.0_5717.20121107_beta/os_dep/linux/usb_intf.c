...
//###########################begin added by rubbit#########################################
#include <mach/memblock.h>
#define THE_MB_USER "rtl-8188-wifi"
extern unsigned long mb_do_alloc(unsigned long size, pid_t tgid, char *name);
extern int mb_do_free(unsigned long addr, pid_t tgid, char *name);
#define mb_alloc(s)				mb_do_alloc(s,MB_DEF_TGID,THE_MB_USER)
#define mb_free(p)				mb_do_free(p,MB_DEF_TGID,THE_MB_USER)

void *wmt_usb_alloc_coherent(struct usb_device *dev, size_t size, gfp_t mem_flags, dma_addr_t *dma) {
	//printk("use wmt mb_alloc,size:%x\n",size);
	unsigned int addr = mb_alloc(size);
	*dma = (dma_addr_t)addr;
	return (void*)mb_phys_to_virt(addr);
}

void wmt_usb_free_coherent(struct usb_device *dev, size_t size, void *addr, dma_addr_t dma) {
	//printk("use wmt mb_free,addr:%x,dma:%x\n",addr,dma);
	if(addr != 0)
		 mb_free((unsigned long)mb_virt_to_phys(addr));
}
//######################################added end#####################################################
...
static int rtw_drv_init(struct usb_interface *pusb_intf, const struct usb_device_id *did)
{
	...
	set_wifi_name("rt3070sta_rtl8188_beta.ko");
