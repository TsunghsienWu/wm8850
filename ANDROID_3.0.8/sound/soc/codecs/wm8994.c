...
static int wm8994_resume(struct snd_soc_codec *codec)
{
	...
	/*struct wm8994_pdata *pdata = control->dev->platform_data;

	if (pdata) {
		// restore GPIO configuration
		for (i = 0; i < ARRAY_SIZE(pdata->gpio_defaults); i++) {
			if (pdata->gpio_defaults[i]) {
				wm8994_set_bits(control, WM8994_GPIO_1 + i,
						0xffff,
		...
		int count, int *eof, void *data)
{
	...
	for (reg = 0; reg < WM8994_CACHE_SIZE; reg++) {
		ret = wm8994_reg_read(wm8994->codec->control_data, reg);
		if (ret < 0) {
...
static int  wm8994_codec_remove(struct snd_soc_codec *codec)
{
	...
	remove_proc_entry("wm8994_dumpregs", NULL);
