	...
	struct snd_pcm_hw_params *params)
{
	...
	//err = snd_soc_dai_set_fmt(codec_dai, SND_SOC_DAIFMT_PDM);
...
static struct snd_soc_dai_link wmt_dai[] = {
	...
	},
	{
		.name = "Voice",
		.stream_name = "Voice",
		.platform_name = "wmt-pdm-pcm.0",
		.cpu_dai_name = "wmt-pdm.0",
...
static int __init wmt_soc_init(void)
{
	...
	if (wmt_getsyspara(varname, buf, &varlen) != 0) {
		...
		sscanf(buf, "vt1603:%x", &i2cbus);
		//info("*** i2c_bus=%x ***", i2cbus);
	}
	...
	wmt_i2s_rate = (SNDRV_PCM_RATE_48000 | SNDRV_PCM_RATE_8000);
