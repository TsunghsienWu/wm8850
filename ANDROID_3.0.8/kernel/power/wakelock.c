...
//--> added by howayhuo for force suspend
static DEFINE_SPINLOCK(timer_lock);
static struct timer_list   s_forceSuspend_timer;
#define FORCE_SUSPEND_INTERVAL 5000  //5 s
extern int g_force_suspend;

static void force_suspend(struct work_struct *work)
{
	unsigned long irqflags;
	printk("force_suspend: enter\n");
	if(g_force_suspend)
	{
		sys_sync();
		...
		//continue suspend util application write the 'on' to /sys/power/state
		spin_lock_irqsave(&timer_lock, irqflags);
		if(g_force_suspend)
			mod_timer(&s_forceSuspend_timer, jiffies + msecs_to_jiffies(FORCE_SUSPEND_INTERVAL));
		spin_unlock_irqrestore(&timer_lock, irqflags);	

	}
	printk("force_suspend: exit\n");

	return;
}

static DECLARE_WORK(force_suspend_work, force_suspend);


void enter_force_suspend(void)
{
	unsigned long irqflags;
	spin_lock_irqsave(&list_lock, irqflags);
	queue_work(suspend_work_queue, &force_suspend_work);
	spin_unlock_irqrestore(&list_lock, irqflags);	
}

static void enter_force_suspend_again(unsigned long fcontext)
{
	unsigned long irqflags;
	
	printk("enter_force_suspend_again\n");
...
static void init_force_suspend_timer(void)
{
	init_timer(&s_forceSuspend_timer);
	s_forceSuspend_timer.function = enter_force_suspend_again;
	s_forceSuspend_timer.data = 0;
}


void cancel_force_suspend_timer(void)
{
	unsigned long irqflags;

	printk("cancel_force_suspend_timer\n");
	spin_lock_irqsave(&timer_lock, irqflags);
	del_timer(&s_forceSuspend_timer);
	spin_unlock_irqrestore(&timer_lock, irqflags);	
}
